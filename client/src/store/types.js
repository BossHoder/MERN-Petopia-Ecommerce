// Original types from client
export const LOGIN_WITH_OAUTH_LOADING = 'LOGIN_WITH_OAUTH_LOADING';
export const LOGIN_WITH_OAUTH_SUCCESS = 'LOGIN_WITH_OAUTH_SUCCESS';
export const LOGIN_WITH_OAUTH_FAIL = 'LOGIN_WITH_OAUTH_FAIL';

export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS';

export const LOGIN_WITH_EMAIL_LOADING = 'LOGIN_WITH_EMAIL_LOADING';
export const LOGIN_WITH_EMAIL_SUCCESS = 'LOGIN_WITH_EMAIL_SUCCESS';
export const LOGIN_WITH_EMAIL_FAIL = 'LOGIN_WITH_EMAIL_FAIL';

export const REGISTER_WITH_EMAIL_LOADING = 'REGISTER_WITH_EMAIL_LOADING';
export const REGISTER_WITH_EMAIL_SUCCESS = 'REGISTER_WITH_EMAIL_SUCCESS';
export const REGISTER_WITH_EMAIL_FAIL = 'REGISTER_WITH_EMAIL_FAIL';

export const ME_LOADING = 'ME_LOADING';
export const ME_SUCCESS = 'ME_SUCCESS';
export const ME_FAIL = 'ME_FAIL';

export const GET_FEATURE = 'GET_FEATURE';
export const GET_PROFILE = 'GET_PROFILE';

export const GET_MESSAGES_LOADING = 'GET_MESSAGES_LOADING';
export const GET_MESSAGES_SUCCESS = 'GET_MESSAGES_SUCCESS';
export const GET_MESSAGES_FAIL = 'GET_MESSAGES_FAIL';

export const ADD_MESSAGE_LOADING = 'ADD_MESSAGE_LOADING';
export const ADD_MESSAGE_SUCCESS = 'ADD_MESSAGE_SUCCESS';
export const ADD_MESSAGE_FAIL = 'ADD_MESSAGE_FAIL';

export const DELETE_MESSAGE_LOADING = 'DELETE_MESSAGE_LOADING';
export const DELETE_MESSAGE_SUCCESS = 'DELETE_MESSAGE_SUCCESS';
export const DELETE_MESSAGE_FAIL = 'DELETE_MESSAGE_FAIL';

export const EDIT_MESSAGE_LOADING = 'EDIT_MESSAGE_LOADING';
export const EDIT_MESSAGE_SUCCESS = 'EDIT_MESSAGE_SUCCESS';
export const EDIT_MESSAGE_FAIL = 'EDIT_MESSAGE_FAIL';
export const CLEAR_MESSAGE_ERROR = 'CLEAR_MESSAGE_ERROR';

export const GET_USERS_LOADING = 'GET_USERS_LOADING';
export const GET_USERS_SUCCESS = 'GET_USERS_SUCCESS';
export const GET_USERS_FAIL = 'GET_USERS_FAIL';

export const GET_PROFILE_LOADING = 'GET_PROFILE_LOADING';
export const GET_PROFILE_SUCCESS = 'GET_PROFILE_SUCCESS';
export const GET_PROFILE_FAIL = 'GET_PROFILE_FAIL';

export const EDIT_USER_LOADING = 'EDIT_USER_LOADING';
export const EDIT_USER_SUCCESS = 'EDIT_USER_SUCCESS';
export const EDIT_USER_FAIL = 'EDIT_USER_FAIL';

export const DELETE_USER_LOADING = 'DELETE_USER_LOADING';
export const DELETE_USER_SUCCESS = 'DELETE_USER_SUCCESS';
export const DELETE_USER_FAIL = 'DELETE_USER_FAIL';

export const RESEED_DATABASE_LOADING = 'RESEED_DATABASE_LOADING';
export const RESEED_DATABASE_SUCCESS = 'RESEED_DATABASE_SUCCESS';
export const RESEED_DATABASE_FAIL = 'RESEED_DATABASE_FAIL';

// Product Types
export const GET_FEATURED_PRODUCTS_LOADING = 'GET_FEATURED_PRODUCTS_LOADING';
export const GET_FEATURED_PRODUCTS_SUCCESS = 'GET_FEATURED_PRODUCTS_SUCCESS';
export const GET_FEATURED_PRODUCTS_FAIL = 'GET_FEATURED_PRODUCTS_FAIL';

export const GET_BESTSELLERS_LOADING = 'GET_BESTSELLERS_LOADING';
export const GET_BESTSELLERS_SUCCESS = 'GET_BESTSELLERS_SUCCESS';
export const GET_BESTSELLERS_FAIL = 'GET_BESTSELLERS_FAIL';

export const GET_ALL_PRODUCTS_LOADING = 'GET_ALL_PRODUCTS_LOADING';
export const GET_ALL_PRODUCTS_SUCCESS = 'GET_ALL_PRODUCTS_SUCCESS';
export const GET_ALL_PRODUCTS_FAIL = 'GET_ALL_PRODUCTS_FAIL';

export const GET_PRODUCT_BY_ID_LOADING = 'GET_PRODUCT_BY_ID_LOADING';
export const GET_PRODUCT_BY_ID_SUCCESS = 'GET_PRODUCT_BY_ID_SUCCESS';
export const GET_PRODUCT_BY_ID_FAIL = 'GET_PRODUCT_BY_ID_FAIL';

// Category Types
export const GET_FEATURED_CATEGORIES_LOADING = 'GET_FEATURED_CATEGORIES_LOADING';
export const GET_FEATURED_CATEGORIES_SUCCESS = 'GET_FEATURED_CATEGORIES_SUCCESS';
export const GET_FEATURED_CATEGORIES_FAIL = 'GET_FEATURED_CATEGORIES_FAIL';

export const GET_ALL_CATEGORIES_LOADING = 'GET_ALL_CATEGORIES_LOADING';
export const GET_ALL_CATEGORIES_SUCCESS = 'GET_ALL_CATEGORIES_SUCCESS';
export const GET_ALL_CATEGORIES_FAIL = 'GET_ALL_CATEGORIES_FAIL';

export const GET_PARENT_CATEGORIES_LOADING = 'GET_PARENT_CATEGORIES_LOADING';
export const GET_PARENT_CATEGORIES_SUCCESS = 'GET_PARENT_CATEGORIES_SUCCESS';
export const GET_PARENT_CATEGORIES_FAIL = 'GET_PARENT_CATEGORIES_FAIL';

export const CLEAR_PRODUCT_ERRORS = 'CLEAR_PRODUCT_ERRORS';
export const CLEAR_CATEGORY_ERRORS = 'CLEAR_CATEGORY_ERRORS';

// New types from demo project
// Cart action types
export const CART_GET_SUCCESS = 'CART_GET_SUCCESS';
export const CART_GET_FAIL = 'CART_GET_FAIL';
export const ADD_TO_CART_SUCCESS = 'ADD_TO_CART_SUCCESS';
export const ADD_TO_CART_FAIL = 'ADD_TO_CART_FAIL';
export const UPDATE_CART_ITEM_SUCCESS = 'UPDATE_CART_ITEM_SUCCESS';
export const UPDATE_CART_ITEM_FAIL = 'UPDATE_CART_ITEM_FAIL';
export const REMOVE_FROM_CART_SUCCESS = 'REMOVE_FROM_CART_SUCCESS';
export const REMOVE_FROM_CART_FAIL = 'REMOVE_FROM_CART_FAIL';
export const CLEAR_CART_SUCCESS = 'CLEAR_CART_SUCCESS';
export const CLEAR_CART_FAIL = 'CLEAR_CART_FAIL';
export const CART_LOADING = 'CART_LOADING';

// Wishlist action types
export const GET_WISHLIST_LOADING = 'GET_WISHLIST_LOADING';
export const GET_WISHLIST_SUCCESS = 'GET_WISHLIST_SUCCESS';
export const GET_WISHLIST_FAILURE = 'GET_WISHLIST_FAILURE';
export const ADD_TO_WISHLIST_SUCCESS = 'ADD_TO_WISHLIST_SUCCESS';
export const ADD_TO_WISHLIST_FAIL = 'ADD_TO_WISHLIST_FAIL';
export const REMOVE_FROM_WISHLIST_SUCCESS = 'REMOVE_FROM_WISHLIST_SUCCESS';
export const REMOVE_FROM_WISHLIST_FAIL = 'REMOVE_FROM_WISHLIST_FAIL';

// Filter action types
export const SET_PRODUCT_FILTERS = 'SET_PRODUCT_FILTERS';
export const CLEAR_PRODUCT_FILTERS = 'CLEAR_PRODUCT_FILTERS';
export const SET_PRODUCT_SORT = 'SET_PRODUCT_SORT';

// Order action types
export const ORDER_CREATE_REQUEST = 'ORDER_CREATE_REQUEST';
export const ORDER_CREATE_SUCCESS = 'ORDER_CREATE_SUCCESS';
export const ORDER_CREATE_FAIL = 'ORDER_CREATE_FAIL';
export const ORDER_CREATE_RESET = 'ORDER_CREATE_RESET';

export const ORDER_DETAILS_REQUEST = 'ORDER_DETAILS_REQUEST';
export const ORDER_DETAILS_SUCCESS = 'ORDER_DETAILS_SUCCESS';
export const ORDER_DETAILS_FAIL = 'ORDER_DETAILS_FAIL';

export const ORDER_PAY_REQUEST = 'ORDER_PAY_REQUEST';
export const ORDER_PAY_SUCCESS = 'ORDER_PAY_SUCCESS';
export const ORDER_PAY_FAIL = 'ORDER_PAY_FAIL';
export const ORDER_PAY_RESET = 'ORDER_PAY_RESET';

export const ORDER_LIST_MY_REQUEST = 'ORDER_LIST_MY_REQUEST';
export const ORDER_LIST_MY_SUCCESS = 'ORDER_LIST_MY_SUCCESS';
export const ORDER_LIST_MY_FAIL = 'ORDER_LIST_MY_FAIL';
export const ORDER_LIST_MY_RESET = 'ORDER_LIST_MY_RESET';

// Address action types
export const GET_ADDRESSES_LOADING = 'GET_ADDRESSES_LOADING';
export const GET_ADDRESSES_SUCCESS = 'GET_ADDRESSES_SUCCESS';
export const GET_ADDRESSES_FAILURE = 'GET_ADDRESSES_FAILURE';
export const ADD_ADDRESS_SUCCESS = 'ADD_ADDRESS_SUCCESS';
export const ADD_ADDRESS_FAIL = 'ADD_ADDRESS_FAIL';

export const UPDATE_ADDRESS_REQUEST = 'UPDATE_ADDRESS_REQUEST';
export const UPDATE_ADDRESS_SUCCESS = 'UPDATE_ADDRESS_SUCCESS';
export const UPDATE_ADDRESS_FAIL = 'UPDATE_ADDRESS_FAIL';

export const DELETE_ADDRESS_REQUEST = 'DELETE_ADDRESS_REQUEST';
export const DELETE_ADDRESS_SUCCESS = 'DELETE_ADDRESS_SUCCESS';
export const DELETE_ADDRESS_FAIL = 'DELETE_ADDRESS_FAIL';

export const SET_DEFAULT_ADDRESS_REQUEST = 'SET_DEFAULT_ADDRESS_REQUEST';
export const SET_DEFAULT_ADDRESS_SUCCESS = 'SET_DEFAULT_ADDRESS_SUCCESS';
export const SET_DEFAULT_ADDRESS_FAIL = 'SET_DEFAULT_ADDRESS_FAIL';

// UI action types
export const SET_LOADING = 'SET_LOADING';
export const SET_ERROR = 'SET_ERROR';
export const CLEAR_ERROR = 'CLEAR_ERROR';

// Review action types
export const GET_REVIEWS_REQUEST = 'GET_REVIEWS_REQUEST';
export const GET_REVIEWS_SUCCESS = 'GET_REVIEWS_SUCCESS';
export const GET_REVIEWS_FAIL = 'GET_REVIEWS_FAIL';

export const ADD_REVIEW_REQUEST = 'ADD_REVIEW_REQUEST';
export const ADD_REVIEW_SUCCESS = 'ADD_REVIEW_SUCCESS';
export const ADD_REVIEW_FAIL = 'ADD_REVIEW_FAIL';
export const ADD_REVIEW_RESET = 'ADD_REVIEW_RESET';

// Checkout state management types
export const PRESERVE_CHECKOUT_STATE = 'PRESERVE_CHECKOUT_STATE';
export const RESTORE_CHECKOUT_STATE = 'RESTORE_CHECKOUT_STATE';
export const CLEAR_CHECKOUT_STATE = 'CLEAR_CHECKOUT_STATE';
export const UPDATE_CHECKOUT_STEP = 'UPDATE_CHECKOUT_STEP';
export const UPDATE_PAYMENT_METHOD = 'UPDATE_PAYMENT_METHOD';
export const UPDATE_SHIPPING_ADDRESS = 'UPDATE_SHIPPING_ADDRESS';

// Breadcrumb action types
export const SET_BREADCRUMB_ITEMS = 'SET_BREADCRUMB_ITEMS';
export const CLEAR_BREADCRUMB_ITEMS = 'CLEAR_BREADCRUMB_ITEMS';
export const UPDATE_BREADCRUMB_LOADING = 'UPDATE_BREADCRUMB_LOADING';
export const SET_BREADCRUMB_ERROR = 'SET_BREADCRUMB_ERROR';
export const CLEAR_BREADCRUMB_ERROR = 'CLEAR_BREADCRUMB_ERROR';

// Admin action types
export const ADMIN_DASHBOARD_STATS_REQUEST = 'ADMIN_DASHBOARD_STATS_REQUEST';
export const ADMIN_DASHBOARD_STATS_SUCCESS = 'ADMIN_DASHBOARD_STATS_SUCCESS';
export const ADMIN_DASHBOARD_STATS_FAIL = 'ADMIN_DASHBOARD_STATS_FAIL';

export const ADMIN_ORDERS_REQUEST = 'ADMIN_ORDERS_REQUEST';
export const ADMIN_ORDERS_SUCCESS = 'ADMIN_ORDERS_SUCCESS';
export const ADMIN_ORDERS_FAIL = 'ADMIN_ORDERS_FAIL';

export const ADMIN_ORDER_UPDATE_REQUEST = 'ADMIN_ORDER_UPDATE_REQUEST';
export const ADMIN_ORDER_UPDATE_SUCCESS = 'ADMIN_ORDER_UPDATE_SUCCESS';
export const ADMIN_ORDER_UPDATE_FAIL = 'ADMIN_ORDER_UPDATE_FAIL';

export const ADMIN_ORDER_DETAILS_REQUEST = 'ADMIN_ORDER_DETAILS_REQUEST';
export const ADMIN_ORDER_DETAILS_SUCCESS = 'ADMIN_ORDER_DETAILS_SUCCESS';
export const ADMIN_ORDER_DETAILS_FAIL = 'ADMIN_ORDER_DETAILS_FAIL';

export const ADMIN_USERS_REQUEST = 'ADMIN_USERS_REQUEST';
export const ADMIN_USERS_SUCCESS = 'ADMIN_USERS_SUCCESS';
export const ADMIN_USERS_FAIL = 'ADMIN_USERS_FAIL';

export const ADMIN_USER_DETAILS_REQUEST = 'ADMIN_USER_DETAILS_REQUEST';
export const ADMIN_USER_DETAILS_SUCCESS = 'ADMIN_USER_DETAILS_SUCCESS';
export const ADMIN_USER_DETAILS_FAIL = 'ADMIN_USER_DETAILS_FAIL';

export const ADMIN_USER_UPDATE_REQUEST = 'ADMIN_USER_UPDATE_REQUEST';
export const ADMIN_USER_UPDATE_SUCCESS = 'ADMIN_USER_UPDATE_SUCCESS';
export const ADMIN_USER_UPDATE_FAIL = 'ADMIN_USER_UPDATE_FAIL';

export const ADMIN_USER_DELETE_REQUEST = 'ADMIN_USER_DELETE_REQUEST';
export const ADMIN_USER_DELETE_SUCCESS = 'ADMIN_USER_DELETE_SUCCESS';
export const ADMIN_USER_DELETE_FAIL = 'ADMIN_USER_DELETE_FAIL';

export const ADMIN_USERS_BULK_UPDATE_REQUEST = 'ADMIN_USERS_BULK_UPDATE_REQUEST';
export const ADMIN_USERS_BULK_UPDATE_SUCCESS = 'ADMIN_USERS_BULK_UPDATE_SUCCESS';
export const ADMIN_USERS_BULK_UPDATE_FAIL = 'ADMIN_USERS_BULK_UPDATE_FAIL';

// Admin Parent Categories Management
export const ADMIN_PARENT_CATEGORIES_REQUEST = 'ADMIN_PARENT_CATEGORIES_REQUEST';
export const ADMIN_PARENT_CATEGORIES_SUCCESS = 'ADMIN_PARENT_CATEGORIES_SUCCESS';
export const ADMIN_PARENT_CATEGORIES_FAIL = 'ADMIN_PARENT_CATEGORIES_FAIL';

export const ADMIN_PARENT_CATEGORY_CREATE_REQUEST = 'ADMIN_PARENT_CATEGORY_CREATE_REQUEST';
export const ADMIN_PARENT_CATEGORY_CREATE_SUCCESS = 'ADMIN_PARENT_CATEGORY_CREATE_SUCCESS';
export const ADMIN_PARENT_CATEGORY_CREATE_FAIL = 'ADMIN_PARENT_CATEGORY_CREATE_FAIL';

export const ADMIN_PARENT_CATEGORY_UPDATE_REQUEST = 'ADMIN_PARENT_CATEGORY_UPDATE_REQUEST';
export const ADMIN_PARENT_CATEGORY_UPDATE_SUCCESS = 'ADMIN_PARENT_CATEGORY_UPDATE_SUCCESS';
export const ADMIN_PARENT_CATEGORY_UPDATE_FAIL = 'ADMIN_PARENT_CATEGORY_UPDATE_FAIL';

export const ADMIN_PARENT_CATEGORY_DELETE_REQUEST = 'ADMIN_PARENT_CATEGORY_DELETE_REQUEST';
export const ADMIN_PARENT_CATEGORY_DELETE_SUCCESS = 'ADMIN_PARENT_CATEGORY_DELETE_SUCCESS';
export const ADMIN_PARENT_CATEGORY_DELETE_FAIL = 'ADMIN_PARENT_CATEGORY_DELETE_FAIL';

// Admin Categories Management
export const ADMIN_CATEGORIES_REQUEST = 'ADMIN_CATEGORIES_REQUEST';
export const ADMIN_CATEGORIES_SUCCESS = 'ADMIN_CATEGORIES_SUCCESS';
export const ADMIN_CATEGORIES_FAIL = 'ADMIN_CATEGORIES_FAIL';

export const ADMIN_CATEGORY_CREATE_REQUEST = 'ADMIN_CATEGORY_CREATE_REQUEST';
export const ADMIN_CATEGORY_CREATE_SUCCESS = 'ADMIN_CATEGORY_CREATE_SUCCESS';
export const ADMIN_CATEGORY_CREATE_FAIL = 'ADMIN_CATEGORY_CREATE_FAIL';

export const ADMIN_CATEGORY_UPDATE_REQUEST = 'ADMIN_CATEGORY_UPDATE_REQUEST';
export const ADMIN_CATEGORY_UPDATE_SUCCESS = 'ADMIN_CATEGORY_UPDATE_SUCCESS';
export const ADMIN_CATEGORY_UPDATE_FAIL = 'ADMIN_CATEGORY_UPDATE_FAIL';

export const ADMIN_CATEGORY_DELETE_REQUEST = 'ADMIN_CATEGORY_DELETE_REQUEST';
export const ADMIN_CATEGORY_DELETE_SUCCESS = 'ADMIN_CATEGORY_DELETE_SUCCESS';
export const ADMIN_CATEGORY_DELETE_FAIL = 'ADMIN_CATEGORY_DELETE_FAIL';

// Admin Products Management
export const ADMIN_PRODUCTS_REQUEST = 'ADMIN_PRODUCTS_REQUEST';
export const ADMIN_PRODUCTS_SUCCESS = 'ADMIN_PRODUCTS_SUCCESS';
export const ADMIN_PRODUCTS_FAIL = 'ADMIN_PRODUCTS_FAIL';

export const ADMIN_PRODUCT_CREATE_REQUEST = 'ADMIN_PRODUCT_CREATE_REQUEST';
export const ADMIN_PRODUCT_CREATE_SUCCESS = 'ADMIN_PRODUCT_CREATE_SUCCESS';
export const ADMIN_PRODUCT_CREATE_FAIL = 'ADMIN_PRODUCT_CREATE_FAIL';

export const ADMIN_PRODUCT_UPDATE_REQUEST = 'ADMIN_PRODUCT_UPDATE_REQUEST';
export const ADMIN_PRODUCT_UPDATE_SUCCESS = 'ADMIN_PRODUCT_UPDATE_SUCCESS';
export const ADMIN_PRODUCT_UPDATE_FAIL = 'ADMIN_PRODUCT_UPDATE_FAIL';

export const ADMIN_PRODUCT_DELETE_REQUEST = 'ADMIN_PRODUCT_DELETE_REQUEST';
export const ADMIN_PRODUCT_DELETE_SUCCESS = 'ADMIN_PRODUCT_DELETE_SUCCESS';
export const ADMIN_PRODUCT_DELETE_FAIL = 'ADMIN_PRODUCT_DELETE_FAIL';

// Admin Coupons Management
export const ADMIN_COUPONS_REQUEST = 'ADMIN_COUPONS_REQUEST';
export const ADMIN_COUPONS_SUCCESS = 'ADMIN_COUPONS_SUCCESS';
export const ADMIN_COUPONS_FAIL = 'ADMIN_COUPONS_FAIL';

export const ADMIN_COUPON_CREATE_REQUEST = 'ADMIN_COUPON_CREATE_REQUEST';
export const ADMIN_COUPON_CREATE_SUCCESS = 'ADMIN_COUPON_CREATE_SUCCESS';
export const ADMIN_COUPON_CREATE_FAIL = 'ADMIN_COUPON_CREATE_FAIL';

export const ADMIN_COUPON_UPDATE_REQUEST = 'ADMIN_COUPON_UPDATE_REQUEST';
export const ADMIN_COUPON_UPDATE_SUCCESS = 'ADMIN_COUPON_UPDATE_SUCCESS';
export const ADMIN_COUPON_UPDATE_FAIL = 'ADMIN_COUPON_UPDATE_FAIL';

export const ADMIN_COUPON_DELETE_REQUEST = 'ADMIN_COUPON_DELETE_REQUEST';
export const ADMIN_COUPON_DELETE_SUCCESS = 'ADMIN_COUPON_DELETE_SUCCESS';
export const ADMIN_COUPON_DELETE_FAIL = 'ADMIN_COUPON_DELETE_FAIL';

// Public Coupons (Customer-facing)
export const COUPONS_REQUEST = 'COUPONS_REQUEST';
export const COUPONS_SUCCESS = 'COUPONS_SUCCESS';
export const COUPONS_FAIL = 'COUPONS_FAIL';

export const COUPON_BY_CODE_REQUEST = 'COUPON_BY_CODE_REQUEST';
export const COUPON_BY_CODE_SUCCESS = 'COUPON_BY_CODE_SUCCESS';
export const COUPON_BY_CODE_FAIL = 'COUPON_BY_CODE_FAIL';

export const COUPON_VALIDATE_REQUEST = 'COUPON_VALIDATE_REQUEST';
export const COUPON_VALIDATE_SUCCESS = 'COUPON_VALIDATE_SUCCESS';
export const COUPON_VALIDATE_FAIL = 'COUPON_VALIDATE_FAIL';

export const COUPON_CLEAR_ERRORS = 'COUPON_CLEAR_ERRORS';

// Checkout Coupon Types
export const CHECKOUT_COUPON_APPLY_REQUEST = 'CHECKOUT_COUPON_APPLY_REQUEST';
export const CHECKOUT_COUPON_APPLY_SUCCESS = 'CHECKOUT_COUPON_APPLY_SUCCESS';
export const CHECKOUT_COUPON_APPLY_FAIL = 'CHECKOUT_COUPON_APPLY_FAIL';

export const CHECKOUT_COUPON_REMOVE = 'CHECKOUT_COUPON_REMOVE';
export const CHECKOUT_COUPON_CLEAR_ERRORS = 'CHECKOUT_COUPON_CLEAR_ERRORS';

export const ADMIN_CLEAR_ERRORS = 'ADMIN_CLEAR_ERRORS';

// ===========================================
// NOTIFICATION ACTION TYPES
// ===========================================

// User Notification Types
export const GET_NOTIFICATIONS_REQUEST = 'GET_NOTIFICATIONS_REQUEST';
export const GET_NOTIFICATIONS_SUCCESS = 'GET_NOTIFICATIONS_SUCCESS';
export const GET_NOTIFICATIONS_FAIL = 'GET_NOTIFICATIONS_FAIL';

export const GET_NOTIFICATION_SUMMARY_REQUEST = 'GET_NOTIFICATION_SUMMARY_REQUEST';
export const GET_NOTIFICATION_SUMMARY_SUCCESS = 'GET_NOTIFICATION_SUMMARY_SUCCESS';
export const GET_NOTIFICATION_SUMMARY_FAIL = 'GET_NOTIFICATION_SUMMARY_FAIL';

export const MARK_NOTIFICATION_READ_REQUEST = 'MARK_NOTIFICATION_READ_REQUEST';
export const MARK_NOTIFICATION_READ_SUCCESS = 'MARK_NOTIFICATION_READ_SUCCESS';
export const MARK_NOTIFICATION_READ_FAIL = 'MARK_NOTIFICATION_READ_FAIL';

export const MARK_ALL_NOTIFICATIONS_READ_REQUEST = 'MARK_ALL_NOTIFICATIONS_READ_REQUEST';
export const MARK_ALL_NOTIFICATIONS_READ_SUCCESS = 'MARK_ALL_NOTIFICATIONS_READ_SUCCESS';
export const MARK_ALL_NOTIFICATIONS_READ_FAIL = 'MARK_ALL_NOTIFICATIONS_READ_FAIL';

export const DELETE_NOTIFICATION_REQUEST = 'DELETE_NOTIFICATION_REQUEST';
export const DELETE_NOTIFICATION_SUCCESS = 'DELETE_NOTIFICATION_SUCCESS';
export const DELETE_NOTIFICATION_FAIL = 'DELETE_NOTIFICATION_FAIL';

// Admin Notification Types
export const ADMIN_NOTIFICATIONS_REQUEST = 'ADMIN_NOTIFICATIONS_REQUEST';
export const ADMIN_NOTIFICATIONS_SUCCESS = 'ADMIN_NOTIFICATIONS_SUCCESS';
export const ADMIN_NOTIFICATIONS_FAIL = 'ADMIN_NOTIFICATIONS_FAIL';

export const ADMIN_NOTIFICATION_BROADCAST_REQUEST = 'ADMIN_NOTIFICATION_BROADCAST_REQUEST';
export const ADMIN_NOTIFICATION_BROADCAST_SUCCESS = 'ADMIN_NOTIFICATION_BROADCAST_SUCCESS';
export const ADMIN_NOTIFICATION_BROADCAST_FAIL = 'ADMIN_NOTIFICATION_BROADCAST_FAIL';

export const ADMIN_NOTIFICATION_STATS_REQUEST = 'ADMIN_NOTIFICATION_STATS_REQUEST';
export const ADMIN_NOTIFICATION_STATS_SUCCESS = 'ADMIN_NOTIFICATION_STATS_SUCCESS';
export const ADMIN_NOTIFICATION_STATS_FAIL = 'ADMIN_NOTIFICATION_STATS_FAIL';

// Real-time Notification Types
export const NOTIFICATION_RECEIVED = 'NOTIFICATION_RECEIVED';
export const CLEAR_NOTIFICATION_ERRORS = 'CLEAR_NOTIFICATION_ERRORS';

// Token management types
export const SET_TOKEN_FROM_STORAGE = 'SET_TOKEN_FROM_STORAGE';
